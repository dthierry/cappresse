...
def fyl_gb(m, it, jt):
    if 0 < jt <= m.ncp_t:
        return m.Gb_l[it, jt] == sum(m.cbin_l[it, jt, c] for c in m.sp)
...
def fyl_tgb(m, it, jt):
    if 0 < jt <= m.ncp_t:
        return m.ebin_l[it, jt] == m.Gb_l[it, jt] * m.cpg_mol * m.Tgb_l[it, jt]
    else:
        return Constraint.Skip

def fyl_yb(m, it, jt, c):
    if 0 < jt <= m.ncp_t:
        return m.cbin_l[it, jt, c] == m.yb_l[it, jt, c] * m.Gb_l[it, jt]
    else:
        return Constraint.Skip
...
def fcp_x_Gb(m, it, kt, ix):
    if 0 < kt <= m.ncp_t and ix < m.nfe_x:
        return m.Gg[it, kt, ix + 1, 0] == \
               sum(m.l1_x[jx] * m.Gg[it, kt, ix, jx] for jx in m.cp_x if jx <= m.ncp_x)
    else:
        return Constraint.Skip
...
def ngb_rule(m, it, jt, ix, jx, c):
    if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
        return m.Ngb[it, jt, ix, jx, c] == \
               m.delta[it, jt, ix, jx] * m.cb[it, jt, ix, jx, c]
    else:
        return Constraint.Skip
...
def hgb_rule(m, it, jt, ix, jx):
    if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
        return m.Hgb[it, jt, ix, jx] == \
               m.delta[it, jt, ix, jx] * sum(m.cb[it, jt, ix, jx, kx] for kx in m.sp) * m.cpg_mol * m.Tgb[it, jt, ix, jx]
    else:
        return Constraint.Skip
...
def dum_dex_vg_rule(m, it, jt, ix, jx):
    if 0 < jt <= m.ncp_t and 0 < jt <= m.ncp_x:
...
def dum_dex_cb_rule(m, it, jt, ix, jx):
    if 0 < jt <= m.ncp_t and 0 < jt <= m.ncp_x:
        return m.dcb_dx[it, jt, ix, jx] == m.hi_x[ix] * m.cbx[it, jt, ix, jx]
