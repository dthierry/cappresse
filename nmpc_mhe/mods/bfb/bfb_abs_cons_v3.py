...
#                sum(m.l1y[jx] * (m.Je[it, jt, m.nfe_x, jx] - m.Jc[it, jt, m.nfe_x, jx]) for jx in m.cp_x if 0 < jx <= m.ncp_x) * m.Ax
#     else:
#         return Constraint.Skip
...
def e25_rule(m, it, jt, j):
    #     # if i == m.ND:
    if 0 < jt <= m.ncp_t:
        return m.ccwin_l[it, jt, j] * m.Ax + m.Sit[it, jt] * (m.nin[j] - m.ne_l[it, jt, j]) == \
               m.cein_l[it, jt, j] * m.Ax
    else:
        return Constraint.Skip
...
def e26_rule(m, it, jt):
    if 0 < jt <= m.ncp_t:
        return m.ecwin_l[it, jt] * m.Ax + m.Sit[it, jt] * (m.hsint[it, jt] - m.hse_l[it, jt]) == \
               m.eein_l[it, jt] * m.Ax
    else:
        return Constraint.Skip
