...
e = MheGen(d_mod=bfb_dae,
           ...
           ref_state=ref_state,
           u_bounds=u_bounds,
           diag_QR=True, nfe_t=nfet, _t=20, ncp_t=3)
...
for i in range(1, 1000):
    ...
    e.lsmhe.report_zL()
    sys.exit()
    # e.lsmhe.pprint(filename="pprint2.txt")
    # sys.exit()
    if stat == 1:

        stat = e.solve_d(e.lsmhe, skip_update=True, iter_max=250, stop_if_nopt=True,
                         jacobian_regularization_value=1e-02, linear_scaling_on_demand=True)
    e.update_state_mhe()
