...
for i in range(1, 1000):
    ...
    try:
        ...
        c.d1.write_nl()
        c.d1.snap_shot(filename="someday_baddie.py")
    ...
    if i == 1:
        val0 = value(c.d1.u1[1])
    if i == 10:
        #     c.create_dyn(initialize=True)
        #     c.solve_d(c.dyn)
        #     c.dyn.write_nl()
        #     sys.exit()
        c.deb_alg_sys_dyn()
        ...
        for x in c.states:
            x_ic = getattr(c.d1, x + "_ic")
            v_tgt = getattr(c.d1, x)
            for ks in x_ic.keys():
                # s = {}
                # s[5] = 1.0
                if type(ks) != tuple:
                    ks = (ks,)
                for j in range(0, c.ncp_t + 1):
                    s = npm(1, 0.1, 6)
                    v_tgt[(1, j) + ks].set_value(value(v_tgt[(1, j) + ks]) * s[5])

        c.solve_d(c.d1, iter_max=5)
        c.d1.write_nl()
        sys.exit()

    if i == 28:
        c.d1.snap_shot(filename="someday_.py")

    if i == 28:
        c.d1.write_nl()
    # c.solve_d(c.d1)
    c.curr_u["u1"] = val0 + val0*0.25
    print(value(c.d1.Gb[1,1,1,1]), value(c.d1.Tgb[1,1,1,1]), value(c.d1.u1[1]), end="\n\n\n")
    ...
    for x in c.states:
        ...
        for ks in x_ic.keys():
            ...
            if type(ks) != tuple:
                ks = (ks,)
            x_ic[ks].value = value(v_tgt[(1, c.ncp_t) + ks]) # * s[5]
            v_tgt[(1, c.ncp_t) + ks].set_value(value(v_tgt[(1, c.ncp_t) + ks]))
