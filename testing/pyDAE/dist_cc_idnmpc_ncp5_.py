...
def main():
    ...
    ref_state = {("x", (42,)): .50}  #: high reboiler heat, phase eq.
    ...
    e.SteadyRef.T[:, :].setub(512.4 - 1E-06)
    ...
    e.PlantSample.nu_l[:, :].setlb(0)
    # e.PlantSample.Vm[:, :].setlb(vml)
    # e.PlantSample.Vm[:, :].setub(vmu)
    e.PlantSample.Qc[:].setlb(1E-08)
    e.PlantSample.D[:].setlb(1E-08)
    ...
    e.olnmpc.nu_l[:, :].setlb(0)
    # e.olnmpc.Vm[:, :].setlb(vml)
    # e.olnmpc.Vm[:, :].setub(vmu)

    # e.olnmpc.objfun_nmpc.deactivate()
    e.olnmpc.Qr[:].setlb(0)
    e.olnmpc.Qc[:].setlb(1E-08)
    e.olnmpc.D[:].setlb(1E-08)


    e.olnmpc.s = Var(e.olnmpc.t, within=NonNegativeReals, initialize=0)
    e.olnmpc.q = Var(within=NonNegativeReals, initialize=0)
    e.olnmpc.q.fix()
    #e.olnmpc.purity_con = Constraint(e.olnmpc.t, rule=lambda m, t: (m.x[t, 1] - 0.12) + m.s[t] == m.q if t > 0 else Constraint.Skip)
    e.olnmpc.purity_con = Constraint(e.olnmpc.t,
                                     rule=lambda m, t: m.x[t, 1] <= 0.35 if t > 0 else Constraint.Skip)
    e.olnmpc.purity_con.deactivate()
    e.olnmpc.objfun_ec = Objective(expr=(sum(e.olnmpc.Qr[j] for j in e.olnmpc.t if j > 0)), sense=minimize)
    e.olnmpc.objfun_ec.deactivate()
    ...
    for i in range(0, 4000):
        ...
        if True:
            e.olnmpc.objfun_nmpc.deactivate()
            e.olnmpc.purity_con.activate()
            e.olnmpc.objfun_ec.activate()

            e.preparation_phase_nmpc(as_strategy=False, make_prediction=False, plant_state=True, sbs=False)
            j = 0
            stat_nmpc = 1
            stat_nmpc = e.solve_dyn(e.olnmpc,
                                    stop_if_nopt=False,
