...
def main():
    ...
    plant_param_file = "PlantSample_1517274185_1049.json"
    e.param_reader(e.PlantSample, plant_param_file)
    e.solve_dyn(e.PlantSample, loadsolve=True, solfile="PlantSample_1517274185_1049.sol")
    ...
    # e.shift_mhe()
    # e.init_step_mhe()
    for obj in e.lsmhe.component_objects(Objective):
        obj.deactivate()
    e.regen_objective_fun()  #: Regen erate the obj fun
    e.deact_icc_mhe()  #: Remove the initial conditions
    e.param_reader(e.lsmhe, "LSMHE(Least-SquaresMHE)_1517274185_1049.json")
    e.solve_dyn(e.lsmhe,
                skip_update=False,
                max_cpu_time=600,
                ma57_pre_alloc=5, tag="lsmhe", loadsolve=True, solfile="LSMHE(Least-SquaresMHE)_1517274185_1049.sol")  #: Pre-loaded mhe solve


    #: Create NMPC
    e.create_nmpc()
    e.param_reader(e.olnmpc, "olnmpc.json")
    stat_nmpc = e.solve_dyn(e.olnmpc, skip_update=False, max_cpu_time=300,
                            jacobian_regularization_value=1e-04, tag="olnmpc",
                            loadsolve=True, solfile="olnmpc.sol")
