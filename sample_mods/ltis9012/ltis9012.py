...
class ltiis9016(ConcreteModel):
    def __init__(self, nfe_t, ncp_t, **kwargs):
        ...
        self.nx = Set(initialize=[ii for ii in range(0, 2)])
        self.nu = Set(initialize=[ii for ii in range(0, 1)])
    ...
    def equalize_u(self, direction="u_to_r"):
        ...
        if direction == "u_to_r":
            for i in iterkeys(self.u1):
                self.u[i, 0].set_value(value(self.u1[i]))
