...
from pyomo.core.base import Constraint, Expression
...
# def de_hgc_rule(m, it, jt, ix, jx):
#     if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
#         return (1/m.hi_t[it]) * (m.dHgc_dt[it, jt, ix, jx]) == \
#                m.delta[it, jt, ix, jx]*(m.Ax * m.Hbc[it, jt, ix, jx] * (m.Tgb[it, jt, ix, jx] - m.Tgc[it, jt, ix, jx]) - \
#                 m.Ax * m.Hce[it, jt, ix, jx] * (m.Tgc[it, jt, ix, jx] - m.Tge[it, jt, ix, jx]) - \
#                 m.Ax * m.fcw[it, jt, ix, jx] * (1 - m.ed[it, jt, ix, jx]) * m.rhos * m.ap * exp(m.whp[it, jt, ix, jx]) * (m.Tgc[it, jt, ix, jx] - m.Tsc[it, jt, ix, jx]) - \
#                 m.Ax * m.fcw[it, jt, ix, jx] * (1 - m.ed[it, jt, ix, jx]) * sum(m.rgc[it, jt, ix, jx, k] * m.cpgcgc[k] for k in m.sp2) * m.Tgc[it, jt, ix, jx])
#     else:
