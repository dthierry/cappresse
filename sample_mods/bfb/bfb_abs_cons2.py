...
def i1_rule(m, it, jt, ix, jx, k):
    if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
        return m.Kgbulk[it, jt, ix, jx, k] == m.K_d * (sum(m.ce[it, jt, ix, jx, kx] for kx in m.sp) - m.cbt[it, jt, ix, jx]) * m.yb[
            it, jt, ix, jx, k]
    else:
        return Constraint.Skip
...
def i2_rule(m, it, jt, ix, jx):
    if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
        return m.Hgbulk[it, jt, ix, jx] == m.K_d * (sum(m.ce[it, jt, ix, jx, kx] for kx in m.sp) - m.cbt[it, jt, ix, jx]) * m.cpg_mol * \
                                           m.Tgb[it, jt, ix, jx]
    else:
        return Constraint.Skip
