...
class bfb_dae(ConcreteModel):
    def __init__(self, nfe_t, ncp_t, **kwargs):
        ...
        self.hi_t = hi_t if steady else Param(self.fe_t, initialize=hi_t, mutable=True)
        ...
        self.wvmf = Var(self.fe_t, self.cp_ta, initialize=-4.77, bounds=(-50, 5))
        ...
        self.wcc = Var(self.fe_t, self.cp_ta, self.fe_x, self.cp_x, self.sp, initialize=2.7, bounds=(-50, 5))
        ...
        def a72_rulex(m, it, jt, ix, jx):
            if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
                return (1/m.hi_t[it]) * (m.r3c[it, jt, ix, jx]) == (exp(m.wk3c[it, jt, ix, jx]) * (((1 - 2 * (m.nc[it, jt, ix, jx, 'n'] * m.rhos / m.nv) - (m.nc[it, jt, ix, jx, 'c'] * m.rhos / m.nv)) ** 2) * ((m.P[it, jt, ix, jx] * m.yc[it, jt, ix, jx, 'c'] * 1E5) ** m.m1) - ((m.nc[it, jt, ix, jx, 'n'] * m.rhos / m.nv) * ((m.nc[it, jt, ix, jx, 'n'] * m.rhos / m.nv) + (m.nc[it, jt, ix, jx, 'c'] * m.rhos / m.nv)) /exp(m.wKe3c[it, jt, ix, jx])))) * (1/m.hi_t[it])
            else:
                return Constraint.Skip
        ...
        def a75_rulex(m, it, jt, ix, jx):
            if 0 < jt <= m.ncp_t and 0 < jx <= m.ncp_x:
                return (1/m.hi_t[it]) * (m.r3e[it, jt, ix, jx]) == (exp(m.wk3e[it, jt, ix, jx]) * (((1. - 2. * (m.ne[it, jt, ix, jx, 'n'] * m.rhos / m.nv) - (m.ne[it, jt, ix, jx, 'c'] * m.rhos / m.nv)) ** 2) * (((m.P[it, jt, ix, jx] * m.ye[it, jt, ix, jx, 'c']) ** m.m1) * (1E5 ** m.m1)) - ((m.ne[it, jt, ix, jx, 'n'] * m.rhos / m.nv) * ((m.ne[it, jt, ix, jx, 'n'] * m.rhos / m.nv) + (m.ne[it, jt, ix, jx, 'c'] * m.rhos / m.nv)) /exp(m.wKe3e[it, jt, ix, jx])))) * (1/m.hi_t[it])
            else:
                return Constraint.Skip
