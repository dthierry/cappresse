...
from bfb_abs_cons6vgdt import *
from initial_s_abs import ss
import os, sys
...
class bfb_dae(ConcreteModel):
    def __init__(self, nfe_t, ncp_t, **kwargs):
        ...
        self.HXIn_T = Param(initialize=306.15, mutable=True)  # 33
        self.HXIn_F = Param(initialize=60000, mutable=True)

        # Gas input condition
        self.GasIn_T = Param(self.fe_t, initialize=313.16, mutable=True)  # needs i_value 40C
        ...
        # mol/kg
        self.SolidIn_T = Param(initialize=323.15, mutable=True)
        ...
        self.Gb = Var(self.fe_t, self.cp_t, self.fe_x, self.cp_x, initialize=1.)
        ...
        if self.nfe_t > 1:
            ...
            self.cp_Hse = None if steady else Constraint(self.fe_t, self.fe_x, self.cp_x,
                                                         ...
                                                         if i < m.nfe_t and 0 < jx <= m.ncp_x else Constraint.Skip)
            self.cp_Gb = None if steady else Constraint(self.fe_t, self.fe_x, self.cp_x,
                                                        rule=lambda m, i, ix, jx:
                                                        m.noisy_Gb[i, ix, jx] == 0.0
                                                        if i < m.nfe_t and 0 < jx <= m.ncp_x else Constraint.Skip)
        ...
        # --------------------------------------------------------------------------------------------------------------
        #: Control constraint
        # self.u1_e = Expression(self.fe_t, rule=lambda m, i: 0.0)
