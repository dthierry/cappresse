...
class aftei16(ConcreteModel):
    def __init__(self, nfe_t, ncp_t, **kwargs):
        ...
        # --------------------------------------------------------------------------------------------------------------
        self.x = Var(self.fe_t, self.cp_t, self.nx, initialize=lambda m, i, j: 1.0)

        #: Initial state-Param
        zero2 = dict.fromkeys(self.fe_t * self.cp_t * self.nx)
        zero_x = dict.fromkeys(self.nx)
        self.x_ic = zero_x if steady else Param(initialize=0.0, mutable=True)

        #:  Derivative-var
        self.dx_dt = zero2 if steady else Var(self.fe_t, self.cp_t, self.nx, initialize=0.0)
        self.u1 = Param(self.fe_t, mutable=True)  #: Dummy
        self.u2 = Param(self.fe_t, mutable=True)  #: Dummy
        ...
        def acx(m, nx):
            return m.x[1, 0, nx] == m.x_ic[nx]
        #: Initial condition-Constraints
        self.x_icc = None if steady else Constraint(self.nx, rule=acx)

        self.u1_e = Expression(self.fe_t, rule=lambda m, i: self.u[i, 1])
        self.u2_e = Expression(self.fe_t, rule=lambda m, i: self.u[i, 2])
    ...
    def create_bounds(self):
        ...
        for value in itervalues(self.x):
            value.setlb(1.0)
        for value in itervalues(self.u):
            value.setlb(1.0)
    ...
    def equalize_u(self, direction="u_to_r"):
        ...
        if direction == "u_to_r":
            ...
            for i in iterkeys(self.u1):
                self.u1[i].value = value(self.u[i, 1])
            for i in iterkeys(self.u2):
                self.u2[i].value = value(self.u[i, 2])
